...
<div class="container">
    <div class="card card-primary">
        <div class="card-header">
            <h3 class="card-title">Editar {{$usuario->name}}</h3>
        </div>
        <!-- /.card-header -->
        <!-- form start -->
        <form method="POST" action="{{route('usuarios.update',$usuario)}}" id="form">
            <div class="card-body">
                @csrf
                <input type="hidden" name="json" value="false" />
                <div class="form-group">
                    <x-adminlte-input id="name" label="Nome" name="name" type="text" placeholder="Nome Completo" value="{{$usuario->name}}" />
                    <x-adminlte-input id="email" label="E-mail" name="email" type="email" placeholder="example@email.com" value="{{$usuario->email}}" />
            ...
            <div class="card-footer">
                ...
                <x-adminlte-button type="submit" label="Editar {{$usuario->email}}" class="btn btn-primary" theme="primary" />
        </form>
